...
- **El freelancer (usa la aplicación).** Autoempleado Autónomo en México (diseñador, programador, fotógrafo, consultor…). Trabaja solo, no es técnico y hace varios presupuestos al mes. Sabe manejarse con una web normal; no sabe (ni quiere saber) de impuestos más allá de lo justo.
- **El cliente (no usa la aplicación).** Recibe el PDF por email. Puede ser una **empresa o autónomo**, o un **particular**. La distinción importa: cambia si se aplica o no la retención de ISR.
...
- HU4 — Como freelancer, quiero que la base imponible, el IVA, la retención de ISR (cuando toque) y el total se calculen solos, para no equivocarme con los impuestos.
...
- RF5. El sistema debe calcular automáticamente, en cada presupuesto:
  ...
  - IVA = base imponible × 16 % (tipo por defecto).
  - Retención de ISR (si el freelancer la activa) = base imponible × 15 % o × 7 %, según elija.
  - Total = base imponible + IVA − retención de ISR.
- RF6. Si el cliente es "particular", la retención de ISR no se aplica en ningún caso.
...
- **IVA:** 16 % por defecto (tipo general de servicios profesionales en México).
- **Retención de ISR:** 15 % (general) o 7 % (nuevos autónomos). Es opcional y **solo se aplica a clientes empresa/autónomo, nunca a particulares**.
- **Fórmula del total:** Total = base imponible + IVA − retención de ISR.
...
- CA2. Al activar o desactivar la retención de ISR, o al cambiar entre 15 % y 7 %, el total se recalcula solo.
- CA3. Al marcar el cliente como "particular", la retención de ISR no se aplica y el total sube respecto al mismo presupuesto con retención.
...
- Sin multidivisa: solo Pesos Mexicanos.
...
- PA1. **IVA:** ¿el 16 % debe ser editable? ¿Hacen falta los otros tipos (10 %, 4 %, 0 % exento)?
